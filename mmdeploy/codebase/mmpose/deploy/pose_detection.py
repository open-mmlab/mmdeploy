...
@MMPOSE_TASK.register_module(Task.YOLOX_POSE_DETECTION.value)
class YoloXPoseDetection(BaseTask):
    """Pose detection task class.
    ...
    def build_backend_model(
        ...
        Returns:
            nn.Module: An initialized backend model.
        """
        from .pose_detection_model import build_yolox_pose_model
        data_preprocessor = self.model_cfg.model.data_preprocessor
        if data_preprocessor_updater is not None:
            data_preprocessor = data_preprocessor_updater(data_preprocessor)
        model = build_yolox_pose_model(
            model_files,
            self.model_cfg,
            self.deploy_cfg,
            device=self.device,
