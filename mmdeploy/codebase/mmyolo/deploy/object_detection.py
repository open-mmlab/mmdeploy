...
class ObjectDetection(BaseTask):
    ...
    def get_partition_cfg(partition_type: str) -> Dict:
        ...
        """
        from .model_partition_cfg import MMYOLO_PARTITION_CFG
        assert (partition_type in MMYOLO_PARTITION_CFG), \
            f'Unknown partition_type {partition_type}'
        return MMYOLO_PARTITION_CFG[partition_type]
    ...
    def get_visualizer(self, name: str, save_dir: str):
        ...
        registry = VISUALIZERS._get_root_registry()
        VisualizerClass = registry.get(self.visualizer.type)
        if VisualizerClass.check_instance_created(name):
            return VisualizerClass.get_instance(name)
        else:
            return registry.build_func(self.visualizer, registry=registry)
