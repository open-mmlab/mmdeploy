...
# TODO merge shufflenetv2_backbone__forward__ncnn into a single function
@FUNCTION_REWRITER.register_rewriter(
    func_name='mmcls.models.backbones.shufflenet_v2.InvertedResidual.forward',
    backend='torchscript')
def shufflenetv2_backbone__forward__torchscript(ctx, self, x):
    """Rewrite `forward` of InvertedResidual used in shufflenet_v2 for
    TorchScript backend.

    The chunk in original InvertedResidual.forward will raise error in
    TorchScript.
