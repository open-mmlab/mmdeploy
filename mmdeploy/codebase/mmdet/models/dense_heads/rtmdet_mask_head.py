...
@FUNCTION_REWRITER.register_rewriter(
    func_name='rtmins.head.rtm_condinst_sepbn_head.'
    'RTMCondInsSepBNHead.predict_by_feat')
def rtmcondins_head__predict_by_feat(ctx,
                                 self,
                                 cls_scores: List[Tensor],
                                 bbox_preds: List[Tensor],
                                 kernel_preds: List[Tensor],
                                 mask_feat: Tensor,
                                 seg_logits: Optional[Tensor] = None,
...
def _mask_predict_by_feat_single(self, mask_feat, kernels, priors):
    ...
    weights, biases = parse_dynamic_params(self, kernels)
