...
JNIEXPORT jobject JNICALL CreateByPath(JNIEnv* env, jobject thiz, jstring modelPath,
  ...
  if (status != MM_SUCCESS) {
    __android_log_print(ANDROID_LOG_ERROR, "jni", "failed to create text recognizer, code: %d\n",
                        (int)status);
...
JNIEXPORT jboolean JNICALL Apply(JNIEnv* env, jobject thiz, jobject handlePointer,
  ...
  status = mmdeploy_text_recognizer_apply(text_recognizer, (const mm_mat_t*)pmats, mat_count,
                                          &result_apply);
  if (status != MM_SUCCESS) {
    __android_log_print(ANDROID_LOG_ERROR, "jni", "failed to apply text recognizer, code: %d\n",
                        (int)status);
    return JNI_FALSE;
  }
...
static JNINativeMethod method[] = {
    {"CreateByPath",
     ...
     (bool*)CreateByPath},
    {"Apply",
     "(Lcn/org/openmmlab/mmdeploy/PointerWrapper;Lcn/org/openmmlab/"
     "mmdeploy/PointerWrapper;ILcn/org/openmmlab/mmdeploy/"
     "PointerWrapper;)Z",
     (bool*)Apply},
