...
class YOLOHead : public MMDetection {
 public:
  ...
  int YOLOFeatDecode(const Tensor& feat_map, std::vector<std::vector<int>> anchor, int grid_h, int grid_w,
  ...
  virtual std::vector<float> yolo_decode(float box_x, float box_y, float box_w, float box_h, int stride, std::vector<std::vector<int>> anchor, int j, int i, int a)const =0;
...
public:
  ...
  Result<Value> operator()(const Value& prep_res, const Value& infer_res);
  std::vector<float> yolo_decode(float box_x, float box_y, float box_w, float box_h, int stride, std::vector<std::vector<int>> anchor, int j, int i, int a)const override ;
};
