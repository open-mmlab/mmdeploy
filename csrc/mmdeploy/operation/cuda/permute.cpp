...
class PermuteImpl : public Permute {
 public:
  ...
  Result<void> apply(const Tensor& src, Tensor& dst, const std::vector<int>& axes) override {
    ...
    Buffer _src_strides(device(), sizeof(int) * ndim);
    Buffer _dst_strides(device(), sizeof(int) * ndim);
    ...
    if (src.data_type() == DataType::kINT8) {
      OUTCOME_TRY(PermuteDispatch<uint8_t>(src, dst_tensor, GetNative<int*>(_src_strides),
                                           GetNative<int*>(_dst_strides)));
    } else if (src.data_type() == DataType::kFLOAT) {
      OUTCOME_TRY(PermuteDispatch<float>(src, dst_tensor, GetNative<int*>(_src_strides),
                                         GetNative<int*>(_dst_strides)));
    } else {
  ...
  template <typename T>
  Result<void> PermuteDispatch(const Tensor& src, Tensor& dst, const int* src_strides,
