...
void Tracer::TraceLoad(const std::string &color_type, bool to_float32, TensorShape shape,
...
void Tracer::TraceResize(const std::string &mode, const std::vector<int> &size, DataType dtype) {
  trans_.push_back(ResizeParam{dtype, size, mode});
  state_.shape[1] = size[0];
  state_.shape[2] = size[2];
}

void Tracer::TracePad(float pad_val, const std::vector<int> &tlbr, const std::vector<int> &size,
                      DataType dtype) {
...
void Tracer::TraceNorm(const std::vector<float> &mean, const std::vector<float> &std, bool to_rgb,
                       DataType dtype) {
...
void Tracer::TraceCrop(const std::vector<int> &tlbr, const std::vector<int> &size, DataType dtype) {
  trans_.push_back(CropParam{state_.dtype, tlbr, size});
  state_.shape[1] = size[0];
  state_.shape[2] = size[2];
}

void Tracer::TraceDFB(bool to_float, DataType dtype) {
...
void Tracer::TraceIm2Tensor(DataType dtype) {
  trans_.push_back(HWC2CHWParam{state_.dtype});
  state_.shape = {state_.shape[0], state_.shape[3], state_.shape[1], state_.shape[2]};
}
