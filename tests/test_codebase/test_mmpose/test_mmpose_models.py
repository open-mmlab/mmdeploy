...
def get_conditional_channel_weighting_model():
...
def get_shuffle_unit_model():
    from mmpose.models.backbones.litehrnet import ShuffleUnit

    model = ShuffleUnit(4, 4)
    model.requires_grad_(False)
    return model


def get_stem_model():
    from mmpose.models.backbones.litehrnet import Stem
...
def test_conditional_channel_weighting_forward(backend_type: Backend):
    check_backend(backend_type, True)
    model = get_conditional_channel_weighting_model()
    model.cpu().eval()
...
@pytest.mark.parametrize('backend_type', [Backend.NCNN])
def test_stem_forward(backend_type: Backend):
    check_backend(backend_type, True)
    model = get_stem_model()
    model.cpu().eval()
    imgs = torch.rand(1, 4, 16, 16)
    deploy_cfg = mmcv.Config(
        dict(
            backend_config=dict(type=backend_type.value, use_vulkan=False),
            onnx_config=dict(input_shape=None, output_names=['output']),
...
@pytest.mark.parametrize('backend_type', [Backend.NCNN])
def test_shuffle_unit_forward(backend_type: Backend):
    check_backend(backend_type, True)
    model = get_shuffle_unit_model()
    model.cpu().eval()
    imgs = torch.rand(1, 4, 16, 16)
    deploy_cfg = mmcv.Config(
        dict(
