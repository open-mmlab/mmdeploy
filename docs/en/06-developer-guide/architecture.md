...
├── service                    # Some small boards cannot run python, we use C/S mode for model conversion. Server in this directory
├── tests                         #  unittest
...
- Model conversion mainly depends on tools + mmdeploy + a small part of csrc directory;
- SDK is consist of three directories: csrc + third_party + demo.

## Model Conversion

Here we take ViT of mmcls as an model example, and take ncnn as inference backend example. Other models and inferences are similar.
...
│   │   ├── quant.py                       # Wrap the `ncnn2int8` tool as a python interface
...
3. Run it with real inference, render output, and make sure the result is correct
...
For example, change the process of `conv -> shape -> concat_const -> reshape` to `conv -> reshape` to trim off the redundant `shape` and `concat` operators.
...
When encountering a framework such as snpe that does not support python well, we use C/S mode: encapsulate a server with protocols such as gRPC, and forward the real inference output.
...
mmdeploy also provides FFI in various languages.
