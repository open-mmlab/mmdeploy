...
本篇教程以`mmdeploy-1.0.0rc3-windows-amd64-onnxruntime1.8.1.zip`和`mmdeploy-1.0.0rc3-windows-amd64-cuda11.1-tensorrt8.2.3.0.zip`为例，展示预编译包的使用方法。
...
5. 安装`mmdeploy`（模型转换）以及`mmdeploy_python`（模型推理Python API）的预编译包
   ...
   ```bash
   # 先下载 mmdeploy-1.0.0rc3-windows-amd64-onnxruntime1.8.1.zip
...
5. 安装`mmdeploy`（模型转换）以及`mmdeploy_python`（模型推理Python API）的预编译包
   ...
   ```bash
   # 先下载 mmdeploy-1.0.0rc3-windows-amd64-cuda11.1-tensorrt8.2.3.0.zip
   pip install .\mmdeploy-1.0.0rc3-windows-amd64-cuda11.1-tensorrt8.2.3.0\dist\mmdeploy-1.0.0rc3-py38-none-win_amd64.whl
   pip install .\mmdeploy-1.0.0rc3-windows-amd64-cuda11.1-tensorrt8.2.3.0\sdk\python\mmdeploy_python-1.0.0rc3-cp38-none-win_amd64.whl
   ```
...
1. 编译 examples
   ...
   在`mmdeploy-1.0.0rc3-windows-amd64-onnxruntime1.8.1\sdk\example`目录下
...
2. 添加环境变量或拷贝动态库到exe同级目录
   ...
   若选择添加环境变量，则将`mmdeploy`的运行时库路径（`mmdeploy-1.0.0rc3-windows-amd64-onnxruntime1.8.1\sdk\bin`）添加到PATH，可参考onnxruntime的添加过程。
...
3. 推理：
   ...
   在mmdeploy-1.0.0rc3-windows-amd64-onnxruntime1.8.1\\sdk\\example\\build\\Release目录下：
...
1. 编译 examples
   ...
   在mmdeploy-1.0.0rc3-windows-amd64-cuda11.1-tensorrt8.2.3.0\\sdk\\example目录下
...
2. 添加环境变量或拷贝动态库到exe同级目录
   ...
   若选择添加环境变量，则将`mmdeploy`的运行时库路径（`mmdeploy-1.0.0rc3-windows-amd64-cuda11.1-tensorrt8.2.3.0\sdk\bin`）添加到PATH，可参考onnxruntime的添加过程。
...
3. 推理
   ...
   在mmdeploy-1.0.0rc3-windows-amd64-cuda11.1-tensorrt8.2.3.0\\sdk\\example\\build\\Release目录下：
