...
2. 添加环境变量或拷贝动态库到exe同级目录
   ...
   若选择添加环境变量，则将`mmdeploy`的运行时库路径（`mmdeploy-0.11.0-windows-amd64-onnxruntime1.8.1\sdk\bin`）添加到PATH，可参考onnxruntime的添加过程。
...
2. 添加环境变量或拷贝动态库到exe同级目录
   ...
   若选择添加环境变量，则将`mmdeploy`的运行时库路径（`mmdeploy-0.11.0-windows-amd64-cuda11.1-tensorrt8.2.3.0\sdk\bin`）添加到PATH，可参考onnxruntime的添加过程。
