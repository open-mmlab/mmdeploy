...
namespace Pad {
    ...
    ir::TensorVarPtr Pad(const std::vector<ir::ExprPtr> &shape,
        ...
        condition.push_back(api::logical::lt(iter_vars[0], shape[0] - padding_tlbr[2]));
        condition.push_back(api::logical::ge(iter_vars[1], padding_tlbr[1]));
        condition.push_back(api::logical::lt(iter_vars[1], shape[1] - padding_tlbr[3]));


        return api::compute(
            shape, iter_vars,
            api::if_then_else(
                api::logical::all(condition),
                (*input)(iter_vars[0] - padding_tlbr[0],
                         iter_vars[1] - padding_tlbr[1],
                         iter_vars[2]),
