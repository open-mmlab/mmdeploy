...
int main(int argc, char *argv[]) {
  if (argc < 4) {
    ELENA_WARN("usage: OpFuse <path/of/OpList/json/file> <cpu or cuda> <path/of/generate/code>");
  }
  ...
  bool ResizeOp = std::find(OpList.begin(), OpList.end(), "Resize") != OpList.end() ? true : false;
  ...
  for (int cur_format = 1; cur_format <= 6; cur_format++) {
    for (int interpolation = 1; interpolation <= (ResizeOp ? 2 : 1); interpolation ++) {
      Format format = (Format)cur_format;
      Interpolation ResizeInterpolation = ResizeOp ? (Interpolation)interpolation : Nearest; // if no ResizeOp, use nearest replace
      ...
      auto norm_mean_0 =
          std::make_shared<ir::ScalarVar>("norm_mean_0", ir::ScalarType::Float32);
      auto norm_mean_1 =
          std::make_shared<ir::ScalarVar>("norm_mean_1", ir::ScalarType::Float32);
      auto norm_mean_2 =
          std::make_shared<ir::ScalarVar>("norm_mean_2", ir::ScalarType::Float32);
      auto norm_std_0 =
          std::make_shared<ir::ScalarVar>("norm_std_0", ir::ScalarType::Float32);
      auto norm_std_1 =
          std::make_shared<ir::ScalarVar>("norm_std_1", ir::ScalarType::Float32);
      auto norm_std_2 =
          std::make_shared<ir::ScalarVar>("norm_std_2", ir::ScalarType::Float32);
      auto pad_h =
      ...
      if (std::find(OpList.begin(), OpList.end(), "Normalize") != OpList.end()) {
        std::vector<ir::ExprPtr> mean_vec{norm_mean_0, norm_mean_1, norm_mean_2};
      ...
      for (auto &op : OpList) {
        ...
        if (op == "cvtColorBGR" || op == "cvtColorGRAY") {
          if (CvtFormat == BGR) {
            std::vector<ir::ExprPtr> color_shape{h, w, api::constant<uint64_t>(3)};
            if (format == BGR) {
              continue;
            } else if (format == RGB) {
          ...
          std::vector<ir::ExprPtr> resize_shape{resize_h, resize_w, intermediate->shape->element[2]};
          ...
          std::vector<ir::ExprPtr> crop_shape{
              crop_size,
              crop_size,
              intermediate->shape->element[2]};
          iter_vars = api::construct_indices(crop_shape);
          cur_stage = Crop::Crop(crop_shape, iter_vars, intermediate, crop_top, crop_left);
          ...
          std::vector<ir::ExprPtr> padding_tlbr{pad_top, pad_left, pad_bottom, pad_right};
          ...
          cur_stage = LayoutTrans::HWC2CHW(trans_shape, iter_vars, intermediate);
        } else {
      ...
      if (target == CUDA) {
        ...
        auto t =
            (*sch)[intermediate->op]->split(fuse, api::constant<uint64_t>(512));
      ...
      if (target == CUDA) {
        std::string cuda_code_row = api::genCudaSrc(stmt, arg_list, kernel_name);
        std::string cuda_code = cuda_code_row;
        if (cur_format > 1) cuda_code = Common::delPrelude(cuda_code_row);
        gen_code << cuda_code;
      } else if (target == CPU) {
        std::string x86_code_row = api::genX86Src(stmt, arg_list, kernel_name);
        std::string x86_code = x86_code_row;
        if (cur_format > 1) x86_code = Common::delPrelude(x86_code_row);
        gen_code << x86_code;
      }
    }
  }
  ...
  if (target == CUDA) {
    api::dump_code(gen_code.str(), cc); // TODO
  } else {
    if (ResizeOp) 
      gen_code << Common::bilinear_preprocess_func
               << Common::outer_call_func_begin << Common::bilinear_branch_func << Common::outer_call_func_end;
    else
      gen_code << Common::outer_call_func_begin << Common::outer_call_func_end;
