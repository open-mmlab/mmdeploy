...
namespace Common {
    std::vector<std::vector<float>> NV2BGR_params{
          {1.164, 2.018, 0},
          {1.164, -0.813, -0.391},
          {1.164, 0, 1.596}};  // BGR YUV

    void GenerateChannelAndInput(Format format, Dtype dtype, ir::ExprPtr h, ir::ExprPtr w, ir::ExprPtr &c, ir::TensorVarPtr &img) {
        switch (format) {
            case BGR:
    ...
    ir::TensorVarPtr CastFloat(const std::vector<ir::ExprPtr> &shape,
      ...
      ELENA_ASSERT(ptr_cast<Const<uint64_t>>(shape[2])->get_value() ==
    ...
    ir::TensorVarPtr BGRMean(std::vector<ir::ExprPtr> &mean,
        ...
        return api::compute(
            mean_shape, iter_mean,
            api::if_then_else(C0, mean[0],
                                api::if_then_else(C1, mean[1], mean[2])),
            "BGR_mean");
    }
    ...
    ir::TensorVarPtr GrayMean(std::vector<float> &mean,
        ...
        return api::compute(
            mean_shape, iter_mean, api::constant<float>(mean[0]),
            "Gray_mean");
    }
    ...
    ir::TensorVarPtr BGRStd(std::vector<ir::ExprPtr> &std,
        ...
        return api::compute(
            std_shape, iter_std,
            api::if_then_else(C0, std[0],
                                api::if_then_else(C1, std[1], std[2])),
            "BGR_std");
    }
    ...
    ir::TensorVarPtr GrayStd(std::vector<float> &std,
        ...
        return api::compute(
            std_shape, iter_std, api::constant<float>(std[0]),
            "Gray_std");
    }
    ...
    ir::TensorVarPtr GrayStd(std::vector<ir::ExprPtr> &std,
        ...
        return api::compute(
            std_shape, iter_std, std[0], "Gray_std");
    }

    ir::TensorVarPtr NV2BGRParams(const std::string &name = "NV2BGRParams") {
        ELENA_ASSERT(NV2BGR_params.size() == 3 && NV2BGR_params[0].size() == 3 && NV2BGR_params[1].size() == 3 && NV2BGR_params[2].size() == 3, "BGRStd");

        std::vector<ir::ExprPtr> NV2BGR_params_shape{api::constant<uint64_t>(NV2BGR_params.size()), api::constant<uint64_t>(NV2BGR_params[0].size())};
        ...
        std::vector<ir::ExprPtr> condition_BY; condition_BY.push_back(B); condition_BY.push_back(Y);
        std::vector<ir::ExprPtr> condition_BU; condition_BU.push_back(B); condition_BU.push_back(U);
        std::vector<ir::ExprPtr> condition_BV; condition_BV.push_back(B); condition_BV.push_back(V);
        std::vector<ir::ExprPtr> condition_GY; condition_GY.push_back(G); condition_GY.push_back(Y);
        std::vector<ir::ExprPtr> condition_GU; condition_GU.push_back(G); condition_GU.push_back(U);
        std::vector<ir::ExprPtr> condition_GV; condition_GV.push_back(G); condition_GV.push_back(V);
        std::vector<ir::ExprPtr> condition_RY; condition_RY.push_back(R); condition_RY.push_back(Y);
        std::vector<ir::ExprPtr> condition_RU; condition_RU.push_back(R); condition_RU.push_back(U);
        std::vector<ir::ExprPtr> condition_RV; condition_RV.push_back(R); condition_RV.push_back(V);

        return api::compute(
            NV2BGR_params_shape, iter_NV2BGR_params,
            api::if_then_else(api::logical::all(condition_BY), api::constant<float>(NV2BGR_params[0][0]),
              api::if_then_else(api::logical::all(condition_BU), api::constant<float>(NV2BGR_params[0][1]),
                api::if_then_else(api::logical::all(condition_BV), api::constant<float>(NV2BGR_params[0][2]),
                  api::if_then_else(api::logical::all(condition_GY), api::constant<float>(NV2BGR_params[1][0]),
                    api::if_then_else(api::logical::all(condition_GU), api::constant<float>(NV2BGR_params[1][1]),
                      api::if_then_else(api::logical::all(condition_GV), api::constant<float>(NV2BGR_params[1][2]),
                        api::if_then_else(api::logical::all(condition_RY), api::constant<float>(NV2BGR_params[2][0]),
                          api::if_then_else(api::logical::all(condition_RU), api::constant<float>(NV2BGR_params[2][1]),
                            api::constant<float>(NV2BGR_params[2][2]))))))))),
            "NV2BGR_params");
    }
  ...
  static constexpr const char *bilinear_preprocess_func = R"(
#include <cmath>
...
extern "C" void bilinear_resize_preprocess(uint64_t src_h, uint64_t src_w, uint64_t dst_h, uint64_t dst_w,
  ...
  static constexpr const char *outer_call_func_begin = R"(
...
extern "C" void FuseKernel(uint64_t resize_h, uint64_t resize_w, uint64_t crop_size, int32_t crop_top, int32_t crop_left, float norm_mean_0, float norm_mean_1, float norm_mean_2, float norm_std_0, float norm_std_1, float norm_std_2, uint64_t pad_h, uint64_t pad_w, int32_t pad_top, int32_t pad_left, int32_t pad_bottom, int32_t pad_right, float pad_value, uint8_t* __restrict__ src_raw_data, float* __restrict__ dst_raw_data, uint64_t src_h, uint64_t src_w, const char *format, const char *interpolation = "nearest"){
    ...
    if (resize_h && resize_w && EQUAL(interpolation, "nearest")) {
        if(EQUAL(format, "BGR")){
          BGR_Nearest_Kernel(resize_h, resize_w, crop_size, crop_top, crop_left, norm_mean_0, norm_mean_1, norm_mean_2, norm_std_0, norm_std_1, norm_std_2, pad_h, pad_w, pad_top, pad_left, pad_bottom, pad_right, pad_value, cubfh, cubfw, inth, intw, src_raw_data, dst_raw_data, src_h, src_w);
        } else if(EQUAL(format, "RGB")){
          RGB_Nearest_Kernel(resize_h, resize_w, crop_size, crop_top, crop_left, norm_mean_0, norm_mean_1, norm_mean_2, norm_std_0, norm_std_1, norm_std_2, pad_h, pad_w, pad_top, pad_left, pad_bottom, pad_right, pad_value, cubfh, cubfw, inth, intw, src_raw_data, dst_raw_data, src_h, src_w);
        } else if(EQUAL(format, "GRAY")){
          GRAY_Nearest_Kernel(resize_h, resize_w, crop_size, crop_top, crop_left, norm_mean_0, norm_mean_1, norm_mean_2, norm_std_0, norm_std_1, norm_std_2, pad_h, pad_w, pad_top, pad_left, pad_bottom, pad_right, pad_value, cubfh, cubfw, inth, intw, src_raw_data, dst_raw_data, src_h, src_w);
        } else if(EQUAL(format, "BGRA")){
          BGRA_Nearest_Kernel(resize_h, resize_w, crop_size, crop_top, crop_left, norm_mean_0, norm_mean_1, norm_mean_2, norm_std_0, norm_std_1, norm_std_2, pad_h, pad_w, pad_top, pad_left, pad_bottom, pad_right, pad_value, cubfh, cubfw, inth, intw, src_raw_data, dst_raw_data, src_h, src_w);
        } else if(EQUAL(format, "NV12")){
          NV12_Nearest_Kernel(resize_h, resize_w, crop_size, crop_top, crop_left, norm_mean_0, norm_mean_1, norm_mean_2, norm_std_0, norm_std_1, norm_std_2, pad_h, pad_w, pad_top, pad_left, pad_bottom, pad_right, pad_value, cubfh, cubfw, inth, intw, src_raw_data, dst_raw_data, src_h, src_w);
        } else if(EQUAL(format, "NV21")){
          NV21_Nearest_Kernel(resize_h, resize_w, crop_size, crop_top, crop_left, norm_mean_0, norm_mean_1, norm_mean_2, norm_std_0, norm_std_1, norm_std_2, pad_h, pad_w, pad_top, pad_left, pad_bottom, pad_right, pad_value, cubfh, cubfw, inth, intw, src_raw_data, dst_raw_data, src_h, src_w);
        } else {
           ABORT("This format is not supported");
        }
    } )";    
    
  static constexpr const char *bilinear_branch_func = R"(
 ...
 static constexpr const char *outer_call_func_end = R"(
     else {
