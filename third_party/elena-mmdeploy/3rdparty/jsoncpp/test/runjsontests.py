...
def compareOutputs(expected, actual, message):
  ...
  Expected: '%s'
  Actual:   '%s'
""" % (message, diff_line, safeGetLine(
        expected, diff_line), safeGetLine(actual, diff_line))
...
def runAllTests(jsontest_executable_path,
                input_dir=None,
                use_valgrind=False,
    ...
    if with_json_checker:
        all_test_jsonchecker = glob(
            os.path.join(input_dir, '../jsonchecker', '*.json'))
        ...
        * Test ../jsonchecker/fail27.json
        ...
        known_differences_withjsonchecker = [
            'fail25.json', 'fail13.json', 'fail18.json', 'fail8.json',
            'fail7.json', 'fail10.json', 'fail27.json'
        ]
        test_jsonchecker = [
            test for test in all_test_jsonchecker
    ...
    for input_path in tests + test_jsonchecker:
        ...
        is_json_checker_test = (input_path
                                in test_jsonchecker) or expect_failure
        print('TESTING:', input_path, end=' ')
        options = is_json_checker_test and '--json-checker' or ''
        options += ' --json-writer %s' % writerClass
        cmd = '%s%s %s "%s"' % (valgrind_path, jsontest_executable_path,
                                options, input_path)
        ...
        if is_json_checker_test:
            if expect_failure:
                if not status:
                    print('FAILED')
                    failed_tests.append(
                        (input_path, 'Parsing should have failed:\n%s' %
            ...
            if status:
                ...
                expected_output_path = os.path.splitext(
                    input_path)[0] + '.expected'
                expected_output = open(
                    expected_output_path, 'rt', encoding='utf-8').read()
                detail = (
                    compareOutputs(expected_output, actual_output, 'input')
                    or compareOutputs(expected_output, actual_rewrite_output,
                                      'rewrite'))
                if detail:
                    print('FAILED')
...
def main():
    ...
    parser = OptionParser(
        usage=
        '%prog [options] <path to jsontestrunner.exe> [test case directory]')
    parser.add_option(
        '--valgrind',
        action='store_true',
        dest='valgrind',
    ...
    parser.add_option(
        ...
        default=False,
        help=
        'run all the tests from the official JSONChecker test suite of json.org'
    )
