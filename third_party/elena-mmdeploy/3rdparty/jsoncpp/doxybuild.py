...
def do_subst_in_file(targetfile, sourcefile, dict):
    ...
    For example, if dict is {'%VERSION%': '1.2345', '%BASE%': 'MyProg'}, then
    all instances of %VERSION% in the file will be replaced with 1.2345 etc.
    """
...
def build_doc(options, make_release=False):
    ...
    warning_log_path = os.path.join(output_dir,
                                    '../jsoncpp-doxygen-warning.log')
    ...
    run_doxygen(
        options.doxygen_path, 'doc/doxyfile', 'doc', is_silent=options.silent)
    if not options.silent:
        print(open(warning_log_path, 'r').read())
    index_path = os.path.abspath(
        os.path.join('doc', subst_keys['%HTML_OUTPUT%'], 'index.html'))
    ...
    if options.make_tarball:
        ...
        tarball_sources = [
            output_dir, 'README.md', 'LICENSE', 'NEWS.txt', 'version'
        ]
        tarball_basedir = os.path.join(output_dir, html_output_dirname)
        tarball.make_tarball(tarball_path, tarball_sources, tarball_basedir,
                             html_output_dirname)
...
def main():
    ...
    parser.add_option(
        ...
        help=
        """Path to GraphViz dot tool. Must be full qualified path. [Default: %default]"""
    )
    parser.add_option(
        '--doxygen',
        dest='doxygen_path',
