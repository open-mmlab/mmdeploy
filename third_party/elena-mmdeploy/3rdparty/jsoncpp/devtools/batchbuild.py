...
class BuildDesc:
    ...
    def __repr__(self):
        return 'BuildDesc(%s, build_type=%s)' % (' '.join(
            self.cmake_args()), self.build_type)
...
class BuildData:
    ...
    def _generate_makefiles(self):
        ...
        cmd = ['cmake'
               ] + self.desc.cmake_args() + [os.path.abspath(self.source_dir)]
...
def generate_build_variants(build_descs_by_axis):
    """Returns a list of BuildDesc generated for the partial BuildDesc for each
    axis."""
...
def generate_html_report(html_report_path, builds):
    ...
    for variable in variables:
        ...
        th_vars.append('<th colspan="%d">%s</th>' %
                       (nb_build_type, cgi.escape(' '.join(variable))))
            ...
            for build_type in build_types:
                ...
                if build:
                    ...
                    td = '<td class="%s"><a href="%s" class="%s">CMake: %s</a>' % (
                        build_status.lower(), cmake_log_url,
